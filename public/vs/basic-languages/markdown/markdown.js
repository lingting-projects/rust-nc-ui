...
define("vs/basic-languages/markdown/markdown", ["require", "require"], (require) => {
  ...
  var moduleExports = (() => {
    ...
    var l = (t, e) => {
      ...
      if (e && typeof e == "object" || typeof e == "function") for (let n of c(e)) !i.call(t, n) && n !== o && s(t, n, {
        get: () => e[n],
    ...
    var p = {
      ...
      tokenizer: {
        root: [[/^\s*\|/, "@rematch", "@table_header"], [/^(\s{0,3})(#+)((?:[^\\#]|@escapes)+)((?:#+)?)/, ["white", "keyword", "keyword", "keyword"]], [/^\s*(=+|\-+)\s*$/, "keyword"], [/^\s*((\*[ ]?)+)\s*$/, "meta.separator"], [/^\s*>+/, "comment"], [/^\s*([\*\-+:]|\d+\.)\s/, "keyword"], [/^(\t|[ ]{4})[^ ].*$/, "string"], [/^\s*~~~\s*((?:\w|[\/\-#])+)?\s*$/, {
          ...
          next: "@codeblockgh",
          nextEmbedded: "$1"
        }], [/^\s*```\s*$/, {token: "string", next: "@codeblock"}], {include: "@linecontent"}],
        ...
        codeblock: [[/^\s*~~~\s*$/, {token: "string", next: "@pop"}], [/^\s*```\s*$/, {
          token: "string",
          next: "@pop"
        }], [/.*$/, "variable.source"]],
        codeblockgh: [[/```\s*$/, {token: "string", next: "@pop", nextEmbedded: "@pop"}], [/[^`]+/, "variable.source"]],
        ...
        }], [/<\/(\w+)(\-|\w)*\s*>/, {token: "tag"}], [/<!--/, "comment", "@comment"]],
